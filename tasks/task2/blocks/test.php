...
<body>
    ...
    $count = 0;
    if (isset($_GET['start'])){
        $words = $conn->prepare("SELECT * FROM questions ORDER BY RAND() LIMIT 5");
        $words->execute();
        ...
        foreach ($result as $i => $item){
            ...
            <br>
                <div class="form-check form-check-inline">
                    ...
                    <?php
                    $words = $conn->prepare("SELECT * FROM questions WHERE question != '{$item["question"]}' ORDER BY RAND() LIMIT 1");
                    $words->execute();
                    $result = $words->fetchAll((PDO::FETCH_ASSOC));
                    ...
                    foreach ($result as $item2){
                        ?>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="answer<?php echo $i ?>" value="<?php echo $item2['id'] ?>" required><label class="form-check-label" for="inlineRadio1"><?php echo $item2["answer"] ?></label>
                        </div>
                        <?php
                    }
                    ?>
...
<?php
    if (isset($_GET['submit'])) {
        ...
        for ($i = 0;$i <= 4;$i++){
            ...
            $query0 = $conn->prepare("SELECT id FROM questions WHERE id = '$question'");
            $query0->execute();
            ...
            $sql_insert = "INSERT INTO score(correct_answer)
                        VALUES ('$count_answer')";
            $conn->exec($sql_insert);
        }
